...
// here for the admin dashboard.
export async function trackPageview() {
  const day = todayKey();
  await Promise.all([incr(`analytics:visits:total`), incr(`analytics:visits:day:${day}`)]);
}
...
export async function getStats() {
  ...
  const [pdfTotal, pdfToday, pdfByKind, pdfByItem, topConcours, week, visitsTotal, visitsToday] = await Promise.all([
