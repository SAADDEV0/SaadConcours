...
// Cloudflare's headers are read first, and only then Vercel's. Vercel derives
// x-vercel-ip-* from whoever opened the connection, so the day a proxy is put
// in front of the domain that becomes the proxy's edge node and every visitor
// silently starts reporting from the same wrong city. Cloudflare's own
// cf-ip* headers still describe the actual visitor, so preferring them keeps
// the admin's city view honest either way; with no proxy they are simply
// absent and nothing changes.
// (These require Cloudflare's "Add visitor location headers" managed
// transform, free plan included — without it cf-ipcity is missing and the
// Vercel fallback, which is then correct anyway, takes over.)
export function getClientGeo(req) {
  ...
  const city =
    decode(req.headers.get("cf-ipcity")) ||
...
// A unified feed of every digest-email dispatch attempt — the manual
// composer (/api/admin/send-digest), its "envoyer un test" mode, and the
// unattended daily cron (/api/cron/news-digest). Before this, the cron path
// wrote nothing anywhere: the admin had no way to tell, from the dashboard,
// whether yesterday's automatic alert actually went out or was silently
// skipped (alerts disabled, Gmail not configured, nothing urgent, no
// subscribers) — this is the one place all of that becomes visible.
export async function trackDigestSend(entry) {
  await pushLog(`analytics:log:digest`, { ...entry, at: new Date().toISOString() });
}

export async function getDigestLog(n = 20) {
  return getLog(`analytics:log:digest`, n);
}
