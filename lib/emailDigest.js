...
export function buildEmailText(items, email, customMessage) {
  const lines = items.map((i) => `- ${i.titre}${i.etablissement ? ` (${i.etablissement}${i.ville ? ", " + i.ville : ""})` : ""} — date limite : ${i.date_limite}`);
  const intro = customMessage || `Date limite dans les ${URGENT_DAYS} prochains jours.`;
  const unsubUrl = `${SITE_URL}/api/alerts/unsubscribe?email=${encodeURIComponent(email)}`;
  return [
    "⏰ Ces concours ferment bientôt",
    "",
    intro,
    "",
    ...(lines.length ? lines : ["Aucun concours sélectionné."]),
    "",
