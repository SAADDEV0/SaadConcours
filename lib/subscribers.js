// Email addresses for the "concours ferme bientôt" alert, backed by Upstash
// Redis (same store as lib/analytics.js). These must never land in
...
export async function getSubscriberHistory(days = 14) {
  ...
  // Un seul mget au lieu d'un aller-retour KV par jour.
  let counts;
  if (kvConfigured()) {
    const kv = await getKv();
    counts = await kv.mget(...dates.map((d) => HISTORY_PREFIX + d));
  } else {
    counts = dates.map((d) => (memHistory().has(d) ? memHistory().get(d) : null));
  }
  const out = dates.map((date, i) => ({ date, count: counts[i] === null || counts[i] === undefined ? null : Number(counts[i]) }));
  let last = 0;
  return out.map((p) => {
...
// WITHSCORES call — different @upstash/redis versions have
...
// Liste complète pour l'admin (et l'export CSV vers la plateforme d'emailing
// externe) : chaque adresse avec sa date d'inscription quand elle est connue.
export async function getSubscribersDetailed() {
  ...
  try {
    if (kvConfigured()) {
      const kv = await getKv();
      stamps = normalizeZRangeWithScores(await kv.zrange(TIMESTAMPS_KEY, 0, -1, { withScores: true }));
    } else {
      stamps = [...memTimestamps().entries()].map(([email, subscribedAt]) => ({ email, subscribedAt }));
    }
  } catch (err) {
    console.error("getSubscribersDetailed timestamps failed (non-fatal)", err);
  }
  const at = new Map(stamps.map((s) => [s.email, s.subscribedAt]));
  return emails
    .map((email) => ({ email, subscribedAt: at.get(email) || null }))
    .sort((a, b) => (b.subscribedAt || 0) - (a.subscribedAt || 0) || a.email.localeCompare(b.email));
}
