...
// Filière taxonomy — concours.filiere is free text (see app/concours/page.js,
// the public filter derives its options straight from the data), so nothing
// stops "Marketing" and "marketing" coexisting as two values unless the
// admin has a way to see and merge them.
export async function getFiliereCounts() {
  const list = await concoursStore.getAll();
  const counts = {};
  for (const c of list) {
    const f = (c.filiere || "").trim() || "(sans filière)";
