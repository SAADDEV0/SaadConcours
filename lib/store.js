...
export const getSettings = cache(async () => {
  ...
  // En local, la console écrit dans la surcouche mémoire de lib/githubGit.js
  // et non sur le disque : la relire ici, sinon une bannière ou un réglage
  // enregistré depuis /admin n'apparaît jamais sur le site local.
  const overlay = globalThis.__scAdminGit?.files?.get(SETTINGS_PATH);
  if (overlay) return JSON.parse(Buffer.from(overlay).toString("utf-8"));
