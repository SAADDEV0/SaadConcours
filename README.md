...
- `public/data/extraits/<id>.md` et `public/data/corriges/<id>.md` — une copie lisible par concours de `enonce_md`/`corrige_md` (un fichier `extraits/<id>.md` pour chacun des concours, un fichier `corriges/<id>.md` seulement pour ceux qui ont un corrigé), pour naviguer facilement dans le dépôt GitHub sans ouvrir le JSON — même logique que `public/images/<ville>/<id>/`.
  ⚠️ **Ces fichiers `.md` sont un miroir à sens unique, généré depuis `concours.json` — ils ne sont jamais lus par le site.** Modifier un fichier dans `extraits/` ou `corriges/` sur GitHub n'a aucun effet sur le site (rien n'est mis à jour, rien n'est supprimé non plus) : la seule donnée que l'application lit est `concours.json` au premier chargement, puis Vercel KV. Pour changer un énoncé ou un corrigé effectivement visible sur le site, il faut soit passer par `/admin`, soit éditer `concours.json` et republier via l'API (`PUT /api/concours/<id>`) — puis régénérer les fichiers `.md` correspondants pour qu'ils restent synchronisés.
