...
// `from`/`to` default to the public site's indigo→violet mark; the admin v4
// rail passes its own jade stops so the logo belongs to the panel's palette
// instead of importing the public brand gradient into it.
export default function BrandLogo({ className, gradientId = "adminLogoGrad", from = "#4f46e5", to = "#a855f7" }) {
  return (
    <svg className={className} viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg">
      ...
      <line x1="32" y1="42" x2="32" y2="54" stroke={from} strokeWidth="1.2" />
    </svg>
  );
}
