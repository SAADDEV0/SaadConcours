...
// Hydrates the server-rendered /evaluation page: intercepts a plain click on
// a module card to open the inline QCM instead of letting the browser
// navigate to /evaluation/[id] — that dedicated landing page still exists
// underneath the real <a href> (JS-less visitors, crawlers, ctrl/cmd/middle-click
// all still work), and its "Commencer" button hands back into this quiz via
// ?open=<id>, picked up below. Mirrors ConcoursExplorer.js / CoursExplorer.js.
export default function EvaluationExplorer({ initialData }) {
  useEffect(() => {
    chromeScript();
    ...
    function wireCards() {
      document.querySelectorAll("#evalModuleGrid a.eval-module-card").forEach((card) => {
    ...
    if (openId) {
      const target = ALL.find((m) => m.id === openId && m.available);
